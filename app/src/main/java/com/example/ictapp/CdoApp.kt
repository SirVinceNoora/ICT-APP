...
import com.example.ictapp.ui.components.ByteForgeSplashScreen
...
fun CdoApp() {
    ...
    if (showSplash) {
        ByteForgeSplashScreen(onAnimationFinished = { showSplash = false })
    } else {
        // Database and Settings
        val database = remember { AppDatabase.getDatabase(context) }
        val settingsManager = remember { SettingsManager(context) }

        // Persistent State from Room
        val repairLogs by database.repairDao().getAllLogs().collectAsState(initial = emptyList())
        val deploymentTasks by database.deploymentDao().getAllTasks().collectAsState(initial = emptyList())
        
        // State from DataStore
        val showWelcomePersistent by settingsManager.showWelcome.collectAsState(initial = null)
        ...
        if (showWelcomePersistent == true && showWelcomeSession) {
            WelcomeDialog(
            ...
            BlueGradientBackground {
                Scaffold(
                    ...
                    NavHost(
                        ...
                        composable(Screen.RepairLogs.route) { 
                            RepairLogScreen(
                                logs = repairLogs, 
                        ...
                        composable(Screen.Deployment.route) { 
                            DeploymentScreen(
